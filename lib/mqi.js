...
function loadLibMulti() {
  console.log('entered loadLibMulti function');
  var l;
  var err;
  var p;
  var ldPath;
  ...
  console.log('searchRedistFirst = ', searchRedistFirst);
  if (process.platform === 'win32') {
    ...
    console.log('made it to linux path of library search');
    l='libmqm_r';

    p = path.join(__dirname,"../redist", "lib64");
    
    var files = fs.readdirSync(__dirname);
    console.log('printing out contents of ', __dirname);
    console.log(files);

    var r = path.join(__dirname,"../redist");
    files = fs.readdirSync(r);
    console.log('printing out contents of ', r);
    console.log(files);

    console.log('path = ', p);
    console.log('using the path we think we want here');
    if (searchRedistFirst) {
      err = loadLib(p,l);
      console.log('searchRedistFirst was true to get here.');
    }

    // Try default directory, known from LD_LIBRARY_PATH (setmqenv may have put MQ in there)
    if (err || !searchRedistFirst) {
      console.log('The path ', p, ' did not work');
      console.log('LD_LIBRARY_PATH is ', process.env['LD_LIBRARY_PATH']);
      err = loadLib(null,l);
    }
    // Try standard installation directories for MQ
    if (err) {
      console.log('that also did not work');
      err = loadLib("/opt/mqm/lib64",l);
    }
    if (err) {
      console.log('/opt/mqm/lib64 didnt work');
       err = loadLib("/opt/mqm/lib64/compat",l);
    }
    if (err) {
      console.log('/opt/mqm/lib64/compat didnt work either');
      err = loadLib("/opt/mqm/lib",l);
    }
    if (err && !searchRedistFirst) {
      console.log('code is having a last try');
      err = loadLib(p,l);
    }
  }
  if (err) {
